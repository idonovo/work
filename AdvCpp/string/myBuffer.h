...
public:
    ...
    T*			GetBuffer()const;
    const T&	operator[](std::size_t _pos)const;//{return GetElement(_pos);}
    T&			operator[](std::size_t _pos);//{return GetElement(_pos);}
...
template <class T>
Buffer_t<T>::Buffer_t():m_array (new T[1]), m_capacity(1), m_size(1){}

template <class T>
Buffer_t<T>::Buffer_t(std::size_t _size):m_array (new T[_size]), m_capacity(_size), m_size(_size)
{}
...
template <class T>
void Buffer_t<T>::Resize(std::size_t _size)
{
